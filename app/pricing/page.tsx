...
const PricingPage = () => {
  ...
  const { user } = useAuth()
  const { generateCheckoutSession, getCustomerPortalUrl, loading: paymentLoading } = usePayment()
        ...
        <div className="isolate mx-auto mt-16 grid max-w-md grid-cols-1 gap-y-8 sm:mt-20 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:gap-x-8">
          {Object.values(PaymentPlanId).map((planId) => (
            ...
            >
              ...
              <CardFooter className="px-8 pb-8 xl:px-10 xl:pb-10">
                {isUserSubscribed && planId === PaymentPlanId.Pro ? (
                  <Button
                    onClick={handleCustomerPortalClick}
                    disabled={paymentLoading || !customerPortalUrl}
                    className="w-full"
                    variant="outline"
                  >

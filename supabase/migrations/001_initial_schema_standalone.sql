...
-- Create users table (Standalone PostgreSQL version - no auth.users reference)
...
-- Enable Row Level Security (Note: RLS requires policies to be created separately)
-- For standalone PostgreSQL, we'll skip RLS and rely on application-level security
